...
const PlayerCard = ({ playerData, isOnRightSide, isMobile = false }) => {
  ...
  return (
    <li
      ...
      className={`flex ${isOnRightSide && `flex-row-reverse`} ${isMobile ? 'text-sm' : ''}`}
    >
      <div
      ...
      >
        ...
        ></div>
        <img
          className={`${isMobile ? 'h-[4rem]' : 'h-[8rem]'} ${isOnRightSide && `scale-x-[-1]`}`}
        ...
        <div className={`flex flex-col relative`}>
          ...
          >
            ...
            {[
              ...
              <div
                key={i}
                className={`rounded-full ${isMobile ? 'w-[4px] h-[4px]' : 'w-[6px] h-[6px]'} bg-radar-primary`}
              ></div>
            ))}
